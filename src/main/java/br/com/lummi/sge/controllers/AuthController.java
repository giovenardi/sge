...
//		this.autenticacaoSoap = autenticacao;
	...
	public void redefinir(Usuario usuario) {
		...
		PessoaFisica pessoaFisica = (PessoaFisica)usuario.getFuncionario().getPessoa();
		if (pessoaFisica == null || pessoaFisica.getCpf() == null || pessoaFisica.getCpf().length() < 14) {
...
//			result.include("error", e.getMessage());
	...
	private void validaUsuarioTrocaSenha(Usuario usuario) throws SgeValidationException {
		if (usuario == null 
				|| usuario.getToken() == null || usuario.getToken().isEmpty()
...
//		try {
//			Proxy proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress("172.25.136.131", 8080));
...
//					"Não foi possível fornecer acesso. Você deve estar autenticado para ter acesso.");
